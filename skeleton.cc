...
void Skeleton::set_a_b_k(syntaxTree::Ast &ast,size_t index,size_t &a,bool &is_plus,size_t &b,int &k){
    ...
    switch (ast_node.label()) {
        ...
        case MINUS:{
            ...
            break;
        case SYMBOL:{
            if(let2term.find(ast_node.ref())!=let2term.end()){set_a_b_k(ast, let2term[ast_node.ref()], a, is_plus, b, k);}
            else{a=ast_node.ref();}
        }
            break;
        default:
...
void Skeleton::convert_term(syntaxTree::Ast &ast, size_t index,bool isAnd) {
  ...
  switch (ast_node.label()) {
    ...
    break;
      ...
      case DISTINCT:{
          is_distinct=true;
          if(isAnd){Clause c;formular_.push_back(c);}
          size_t left=ast[ast_node.left_child()].ref();
          size_t right=ast[ast_node.right_child()].ref();
          Literal l(left,right,-1);
          formular_.back().push_back(l);
      }
          break;
      case EQUAL:{
          if(isAnd){
          ...
          else if(ast[ast_node.left_child()].label()==SYMBOL&&ast[ast_node.right_child()].label()==NUMERAL){//a==k
              size_t left=ast[ast_node.left_child()].ref();
              int key=std::atoi(syntaxTree::symbol_table.str(ast[ast_node.right_child()].ref()).c_str());
              if(let2term.find(left)!=let2term.end()){//represent==k
                  convert_term(ast, let2term[left], isAnd);
                  convert_term(ast, let2term[left], false);
                  formular_.back()[formular_.back().size()-2].invertAB();
                  formular_.back()[formular_.back().size()-2].k=-key;
                  formular_.back().back().k=key;
              }
              else{
              Literal l(left,-1,key);
              formular_.back().push_back(l);
              }
          }
...
//                  if(ast[ast_node.right_child()].label()!=OR)
      ...
      case SYMBOL:{//the original bool var
          if(let2term.find(ast_node.ref())!=let2term.end()){convert_term(ast, let2term[ast_node.ref()], isAnd);}
          else{
          if(isAnd){formular_.push_back(Clause());}
